...
| K 轴 128 | 128 | 0.80 | 通过 |
| K 轴 256 | 256 | 0.93 | 通过 |
| K 轴 512 | 512 | 1.03 | 失败（临界） |
| K 轴 1024 | 1024 | 1.26 | 失败 |
| K 轴 per-channel | >= 768（整行） | 1.22 | 失败 |
...
2. **权重边界位于 K = 256 与 K = 512 之间。** K = 256 仍略优于 MXFP4 对照组
   （0.93 倍）；K = 512 是首个明确失败点（1.03 倍），更大的粒度单调劣化。

两个独立随机种子在所有配置上的差异均在 1% 以内，因此排序与边界位置是稳定的筛查
结果。

激活扫描（权重块固定为 K = 64，W4A4，对照组为 **W4A4** 的 MXFP4 group-32，因为已
公开的 MXFP4 SVDQuant 配置同样量化激活）：

| 激活分组 | 相对 MXFP4 g32 W4A4 的 NMSE 比值 | 筛查结论 |
|---|---:|---|
| 16 | 0.52 | 通过 |
| 32 | 0.60 | 通过 |
| 64 | 0.68 | 通过 |
...
相对 W4A16，激活量化把绝对误差抬高约 3 倍（对照组为 2.0e-2 对 7.0e-3），但在不超过
128 的每个分组上，INT4 激活都**相对**优于 MXFP4 激活。因此激活边界同样位于 128 与
逐 token 之间，W4A4 仅在绝对误差意义上属于更高风险的路径。
...
* **推荐的 INT4 权重契约：一维 K 轴分组，`group_size = 128`。** 其误差为 MXFP4
  ...
  K = 64（0.68 倍）作为保守回退。
* **筛查得到的上界为 K = 256**；K = 512 是首个失败点。步骤 5 应评估 256 与 512
  （并以 128 作为更小的对照），如果仍在考虑二维契约，还应评估 16x16。
* **不推荐二维 M×K 块缩放**：在同等元数据成本下它严格劣于 K 轴分组，且即便是
  16x16 也已经处于临界失败。
* **W4A4**：分组不超过 128 的动态 INT4 激活筛查结果优于对应的 MXFP4 W4A4 对照
  组，而逐 token 激活缩放则不然。激活带来的绝对误差仍约为权重的 3 倍，因此 W4A4
  边界必须先在模型级确认后才能对外承诺。
